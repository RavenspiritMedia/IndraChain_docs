...
This guide goes into detail on how to back up and restore a Indrachain node instance.
...
Indrachain leverages LevelDB as its storage engine.
When starting a Indrachain node, the following sub-folders are created in the specified working directory:
...
It is critical for these folders to be preserved in order for the Indrachain instance to run smoothly.
...
Since the Indrachain uses **LevelDB** for data storage, the node needs to be stopped for the duration of the backup, 
as **LevelDB** doesn't allow for concurrent access to its database files.

Additionally, the Indrachain also does data flushing on close.
...
If any instance of the Indrachain is running, it needs to be stopped in order for step 2 to be successful.
...
### Step 3: Run the Indrachain client while specifying the correct data directory 

In order for the Indrachain to use the restored data directory, at launch, the user needs to specify the path to the 
