...
So far, we've set up bridge to exchange assets/data between Polygon PoS and Indrachain chain. This section will guide you to set up an ERC20 bridge and send tokens between different blockchains.
...
We will send ERC20 Tokens from the Polygon PoS chain to the Indrachain chain.
...
To transfer tokens to Indrachain chains, you will call `deposit`.

```bash
# Start transfer from Polygon PoS to Indrachain chain
...
After the deposit transaction was successful, the relayer will get the event and vote for the proposal. It executes a transaction to send tokens to the recipient account in the Indrachain chain after the required number of votes are submitted. 
...
Once the execution transaction has been successful, you will get tokens in the Indrachain chain.

```bash
# Check the ERC20 balance in Indrachain chain
$ cb-sol-cli erc20 balance \
