...
The relayer will get the event and vote for the proposal. It executes a transaction to send NFTs to the recipient account in the Indrachain chain after the required number of votes are submitted. 
...
You can check the owner of the NFT on the Indrachain network after the execution is completed.

```bash
# Check the owner of NFT 0x50 in Indrachain chain
$ cb-sol-cli erc721 owner \
  --url http://localhost:10002 \
  --erc721Address "[ERC721_CONTRACT_ADDRESS]" \
  --id 0x50
