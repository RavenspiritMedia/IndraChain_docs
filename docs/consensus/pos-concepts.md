...
implementation of the Indrachain.

The Indrachain Proof of Stake (PoS) implementation is meant to be an alternative to the existing PoA IBFT implementation,
...
This contract is pre-deployed whenever a PoS mechanism Indrachain chain is initialized, and is available on the address
`0x0000000000000000000000000000000000001001` from block `0`.

### Epochs

Epochs are a concept introduced with the addition of PoS to the Indrachain.
...
**Epoch Blocks** are a concept introduced in the PoS implementation of IBFT in Indrachain.
...
The default size of an epoch is `100000` blocks in the Indrachain.

## Contract pre-deployment

The Indrachain _pre-deploys_
the [Staking Smart Contract](https://github.com/0xPolygon/staking-contracts/blob/main/contracts/Staking.sol)
