...
This article details the necessary steps to get the Indrachain up and running with a [Hashicorp Vault](https://www.vaultproject.io/) server.
...
Additionally, it is required that the Hashicorp Vault server being used for the Indrachain should have **enabled KV storage**.
...
In order for the Indrachain to be able to seamlessly communicate with the Vault server, it needs to parse an already
...
The Indrachain uses the specified node name to keep track of the secrets it generates and uses on the Vault instance.
