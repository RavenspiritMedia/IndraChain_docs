...
The below guide will instruct you on how to set up a Indrachain network on your local machine for testing and development
purposes.

The procedure differs greatly from the way you would want to set up Indrachain network for a real use scenario on
...
In this guide, our goal is to establish a working `Indrachain` blockchain network working with [IBFT consensus protocol](https://github.com/ethereum/EIPs/issues/650).
...
`bootnode` is not a special instance of the Indrachain node. Every Indrachain node can serve as a `bootnode`, but
every Indra-chain node needs to have a set of bootnodes specified which will be contacted to provide information on how to connect with
...
* The `--ibft-validators-prefix-path` sets the prefix folder path to the one specified which IBFT in Indrachain can
...
Because we are attempting to run a Indrachain network consisting of 4 nodes all on the same machine, we need to take care to 
...
A  node can be started with a set **price limit** for incoming transactions.
...
## Step 5: Interact with the Indrachain network
