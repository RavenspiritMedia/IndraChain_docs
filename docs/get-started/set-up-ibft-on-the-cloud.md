...
The below guide will instruct you on how to set up a Indra chain network on a cloud provider for a production setup of your testnet or mainnet.

If you would like to setup a Indra Chain network locally to quickly test the `polygon-edge` before doing a production-like setup, please refer to
[Local Setup](/docs/get-started/set-up-ibft-locally)
:::

## Requirements

Refer to [Installation](/docs/get-started/installation) to install Indrachain.
...
As the only part of the `Indrachain` that needs to be exposed to other VMs is the libp2p server, simply allowing
...
Each of the 4 nodes will run on their own VM, as the idea of this guide is to give you a fully functional Indrachain network while keeping the validator keys private to ensure a trustless network setup.
...
To get up and running with Indrachain, you need to initialize the data folders, on each node:
...
`bootnode` is not a special instance of a Indrachain node. Every Indrachain node can serve as a `bootnode` and
every Indrachain node needs to have a set of bootnodes specified which will be contacted to provide information on how to connect with
...
```
[SECRETS INIT]
Public key (address) = 0x1Cf56c3Fe032B3F8B49037610Eb74F3d2d0CD66A
BLS Public key       = 0x8845cfbf1a284d8ec5de3ab3a1cf2c46fcc42bfce8c5d62f4a0b827b4feb1995c7c0583e919accef75aee763c437bcdf
Node ID              = 16Uiu2HAmJr9dkKiXGeUXDa4ignMcVEPebJ7djWX5YbqemkEGU5d1
...
````bash
polygon-edge genesis --consensus ibft --ibft-validator 0x1Cf56c3Fe032B3F8B49037610Eb74F3d2d0CD66A:0x8845cfbf1a284d8ec5de3ab3a1cf2c46fcc42bfce8c5d62f4a0b827b4feb1995c7c0583e919accef75aee763c437bcdf --ibft-validator <Public key (address)>:<BLS Public key> --ibft-validator <Public key (address)>:<BLS Public key> --ibft-validator <Public key (address)>:<BLS Public key> --bootnode=/ip4/127.0.0.1/tcp/10001/p2p/16Uiu2HAmJr9dkKiXGeUXDa4ignMcVEPebJ7djWX5YbqemkEGU5d1 --bootnode /ip4/127.0.0.1/tcp/20001/p2p/<Node ID > --pos --epoch-size 50 --premine=0x1Cf56c3Fe032B3F8B49037610Eb74F3d2d0CD66A:1000000000000000000000 --premine=<Public key (address)>:1000000000000000000000
...
After running the previous commands, you have set up a 4 node Indrachain network, capable of sealing blocks and recovering
from node failure.
...
A Indrachain node can be started with a set **price limit** for incoming transactions.
