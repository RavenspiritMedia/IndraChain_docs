...
This section details the present commands, command flags in the Indrachain, and how they're used.
...
Used to specify the data directory used for storing Indrachain client data. Default: `./test-chain`.
...
Sets the directory for the Indrachain genesis data. Default: `./genesis.json`.
...
| secrets generate         | Generates a secrets manager configuration file which can be parsed by the Indrachain    |
...
Sets the directory for the Indrachain data if the local FS is used.
