...
To accomplish this task, the Indrachain leverages the battle-tested **libp2p** framework.
...
* **Modularity** - libp2p is modular by nature, just like the Indrachain. This gives greater flexibility, especially when parts of the Indrachain need to be swappable

## GRPC

On top of **libp2p**, the Indrachain uses the **GRPC** protocol. <br />
Technically, the Indrachain uses several GRPC protocols, which will be covered later on.

The GRPC layer helps abstract all the request/reply protocols and simplifies the streaming protocols needed for the Indrachain to function.
...
Earlier, we mentioned that the Indrachain leverages several GRPC protocols.<br />
...
The Indrachain also implements several service methods that are used by other nodes on the network. <br />
