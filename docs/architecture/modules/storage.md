...
The Indrachain currently utilizes **LevelDB** for data storage, as well as an **in-memory** data store.

Throughout the Indrachain, when modules need to interact with the underlying data store, 
...
In order to make querying the LevelDB storage deterministic, and to avoid key storage clashing, the Indrachain leverages
